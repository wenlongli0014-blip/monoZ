...
BASE = "/afs/cern.ch/user/l/liwe/hzz2l2nu/dnn/cls_limit_ranking"
...
def load_limit():
    path = os.path.join(ROOT_DIR, "higgsCombine.Expected_dnn_score.AsymptoticLimits.mH125.root")
    ...
    for value, quantile in zip(arr["limit"], arr["quantileExpected"]):
        q = float(quantile)
        if q < 0:
            out["observed_asimov"] = float(value)
...
def draw_limit(values):
    ...
    ax.plot(values["observed_asimov"], y, marker="o", ms=7, color="#ff7f0e", label="Asimov observed")
    ax.text(values["exp_p2"] + 0.035, y, f"{values['exp']:.3f}", va="center", ha="left", fontsize=12)
    ax.set_yticks([0])
    ax.set_yticklabels(["DNN score"], fontsize=13)
    ax.set_xlabel(r"95% CL upper limit on $BR_{inv}$", fontsize=15)
    ax.set_xlim(0, values["exp_p2"] * 1.30)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(PLOT_DIR, "cls_limit_expected_dnn_score.png"), dpi=150)
    fig.savefig(os.path.join(PLOT_DIR, "cls_limit_expected_dnn_score.pdf"))
    plt.close(fig)

    with open(os.path.join(TABLE_DIR, "cls_limit_dnn_score.json"), "w", encoding="utf-8") as handle:
        json.dump(values, handle, indent=2, sort_keys=True)


def load_impact(param):
    path = os.path.join(ROOT_DIR, f"higgsCombine_paramFit_.dnn_score_impact_{param}.MultiDimFit.mH125.root")
...
def parse_initial_fit():
    path = os.path.join(BASE, "logs", "dnn_score_impacts_initial.log")
...
def make_impact_json():
    ...
    data = {
        "POIs": [{"name": "r", "fit": [min(poi_vals), 0.0, max(poi_vals)]}],
        "method": "manual MultiDimFit impact scan on asimovData_1",
        "params": params,
    }
    with open(os.path.join(JSON_DIR, "impacts_dnn_score.json"), "w", encoding="utf-8") as handle:
...
def draw_ranking(data):
    ...
    rhat, rdown, rup = parse_initial_fit()
    fig.text(0.34, 0.84, r"$\bf{CMS}$ $\it{Internal}$", fontsize=18, ha="left")
    fig.text(0.62, 0.84, "Expected_dnn_score", fontsize=13, ha="left")
    fig.text(0.96, 0.84, rf"$\hat{{r}} = {rhat:.2f}^{{+{rup:.2f}}}_{{-{rdown:.2f}}}$", fontsize=14, ha="right")
    ...
    fig.savefig(os.path.join(PLOT_DIR, "ranking_dnn_score.png"), dpi=160)
    fig.savefig(os.path.join(PLOT_DIR, "ranking_dnn_score.pdf"))
    plt.close(fig)
